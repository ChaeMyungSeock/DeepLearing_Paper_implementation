...
class ResNext_bottleneck(nn.Module):
    def __init__(self, in_planes, planes, stride=1, ii_downsample = None):
        ...
        self.conv1 = nn.Conv2d(in_planes, planes*2, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(planes*2)

        # stride = 1, padding = 1이므로, 너비와 높이는 항시 유지됨
        self.conv2 = nn.Conv2d(planes*2, planes*2, kernel_size=3, stride=stride, padding=1, bias=False, groups=32)
        self.bn2 = nn.BatchNorm2d(planes*2)

        self.conv3 = nn.Conv2d(planes*2, planes*self.mul, kernel_size=1, stride=1, padding=0, bias=False)
